...
## Custom Elements Component Structure

Each Custom Element component should be in its own folder with a separate CSS file:
...
Use Parcel's `bundle-text:` import to load CSS as a string and inject it into the document head:

```js
'use strict';

import styles from 'bundle-text:./ComponentName.css';
import { i18n, storage } from '../../services/index.js';

export class ComponentName extends HTMLElement {
    static {
        // Inject component styles into document head (once per component)
...
1. **NO Shadow DOM** - Components render directly in the light DOM
2. **NO `<template>` tags** - Use template literals for HTML
3. **Static initialization block** - Auto-registers component and injects CSS
4. **CSS data attribute** - Use `data-component` to identify injected styles
5. **One-time injection** - CSS is injected once when the class is loaded

### For Base Components

If multiple components share styles, create a base component with static flag:
